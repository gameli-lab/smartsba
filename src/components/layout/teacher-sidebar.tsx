    ...
    <>
      ...
      >
        <div className="flex h-16 items-center justify-center border-b px-2 group-hover:justify-start group-hover:px-4">
          <div className="shrink-0 rounded-md bg-green-100 p-2 text-green-700 dark:bg-green-950/50 dark:text-green-300">
            <Users className="h-5 w-5" />
          </div>
          <span className="ml-3 hidden text-sm font-semibold text-gray-900 group-hover:inline dark:text-gray-100">Teacher</span>
        </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t bg-white/95 backdrop-blur md:hidden dark:border-gray-800 dark:bg-gray-950/95">
        <div className="grid grid-cols-5 gap-1 px-2 py-2">
          {primaryItems.map((item) => {
            const active = isActivePath(item.href)
            return (
              <Link key={item.href} href={item.href} className={cn('flex flex-col items-center justify-center rounded-md px-1 py-2 text-[11px] font-medium', active ? 'text-green-700 dark:text-green-300' : 'text-gray-600 dark:text-gray-400')}>
                <span>{item.icon}</span>
